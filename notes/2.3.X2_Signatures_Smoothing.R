...
# wf-57d094c630 - ARCHS - comparisons
# wf-48a7d142f5 - ARCHS - in silico
# wf-b06f33a12c - STRING - in silico
# wf-77d7f5bbee - STRING - comparisons
...
append_signatures = function(wfid){
  ...
  new_cols <- setNames(rep(list(NA), 5), c("comparison", "fdr", "pvalue", "in_top50", "in_bottom50"))
  
  signature_df = signature_df %>%
    mutate(!!!new_cols) %>%
    mutate(gx_diff_wfid = wfid)
}
archs = append_signatures("wf-f5930577e9")

uploadToBQ(archs, bqdataset = "s05_atopic_dermatitis", tableName = "X2Signatures", disposition = "WRITE_APPEND")
